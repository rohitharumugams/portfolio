    ...
    <article className={`level-page color-${meta?.color ?? 'lime'}`}>
      <header className="level-hero">
        ...
        <div className="level-meta">
          <span>DIFF {stars(meta?.difficulty ?? 3)}</span>
          <span>{project.when}</span>
          <span>{project.statusLabel.toUpperCase()}</span>
        </div>
        <p className="boss">BOSS FIGHT: {meta?.boss}</p>
        ...
        {tab === 'gameover' && (
          <>
            ...
            <div className="media-slot">
              <p>SCREENSHOT REEL — COMING AFTER YOU APPROVE THE BUILD</p>
            </div>
          </>
        )}
      </div>
